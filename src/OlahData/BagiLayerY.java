...
public class BagiLayerY {
    ...
    public BagiLayerY(double[][] data, double skalaDetil){
        ...
        // MENCARI LIST INDEKS LAYER SESUAI DENGAN NILAI Y    
        layerIndeks.add(data[0][1]);
        indeksLayer.add(0);
        int jml = 1;                    // -> jumlah layer
        ...
        for(int i = 0; i < jml; i++){
            ...
            if(newData.length >= 2){
                splineKubik = new SplineKubik(newData, "y", layerIndeks.get(i), 
                        newData[0][0], newData[newData.length - 1][0],skalaDetil);
